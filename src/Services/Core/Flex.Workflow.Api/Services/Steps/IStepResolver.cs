...
namespace Flex.Workflow.Api.Services.Steps
{
    public interface IStepResolver
    {
        ...
        (int currentStage, int totalStages) GetCurrentStage(IEnumerable<WorkflowAction> actions, WorkflowStepsDocument doc);

        // Returns true if given stage is complete with newApprovalCount
        bool IsStageComplete(int stageIndex, IEnumerable<WorkflowAction> actions, WorkflowStepsDocument doc);
